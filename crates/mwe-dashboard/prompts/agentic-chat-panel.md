...
- `wiki_supersede(old_fact_id, new_body)` — replace ONE fact in-place with a corrected body. Owner, ACL, `fact_type` and topics are inherited from the original; only the body changes. Flow for single-fact corrections ("fix the phone number", "I said Tuesday, it was Wednesday"):
