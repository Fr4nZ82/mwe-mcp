...
fn wiki_read() -> Tool {
    read_only(materialize(
        "wiki_read",
        "Read the rendered content of a **specific page** of a wiki for the given sender, with ACL applied. `path` is **required** — name the page you want; there is no default. Any page of the wiki can be named; the only paths refused are the engine's own files — **any** name starting with `_` (`_meta.md`, `_briefing.md`, …) — which are bookkeeping rather than memory. A page path comes from a `wiki_search` / `wiki_navigate` hit's `path` field, or from a `wiki_navigate` fragment's `page`. Must be a safe relative path; an unknown page is `not_found`. Returns `content_rendered_for_sender` plus `redacted_count`, behind the wiki-level visibility gate. For a **standard** wiki, marked regions are redacted per the per-fragment ACL, and what is NOT a fact — headings, the prose between the regions, a `See also:` line, a `[[wiki/page]]` address — is served by how much of that page is yours to read: whole on your own memory, the readable facts alone on somebody else's, section by section on a group's or a subject's, where a heading and its paragraph go with the facts under them. A page with no fact still holding serves nothing. A **smart** wiki is markerless, so the wiki-level gate alone governs and the page passes whole. The `owner` field says whose wiki it is so you can name it naturally to the person («in your wiki»); it is **not** a permission and grants nothing — what may be read was decided region by region before you were handed the page.",
        json!({
            "type": "object",
...
fn wiki_admin_push() -> Tool {
    materialize(
        "wiki_admin_push",
        "Smart-wiki authoritative write (H family). Smart consumers only (`consumer_class=smart`). `mode=create` forges a new SMART wiki at `slug`, under `parent_wiki_id` (required — a smart wiki takes its read audience from the wiki it sits under). A standard wiki is structure the engine raises for itself and is not created here. `mode=upsert` overwrites pages and applies `deletes` on an existing smart-wiki owned by the caller. No server-side LLM — content is taken verbatim. Optional `mark_processed` rides along the push to mark briefing items as `processed_at=NOW()` atomically with the same transaction (used by the smart consumer to close off dashboard comments the push answers). The response returns `authored_refs`: one `[[wiki_id/page]]` breadcrumb per written page — echo these into the next `wiki_ingest_message` (`metadata.authored_refs`) so personal memory links to the page instead of duplicating its body.",
        json!({
            "type": "object",
            "required": ["mode", "pages"],
            "additionalProperties": false,
            "properties": {
                ...
                "parent_wiki_id": { "type": "string", "description": "Required on `create`: the new wiki lands under this one, which is where its read audience comes from — pass your own root wiki." },
                ...
                "smart": { "type": "boolean", "description": "Must be true on create: this API forges smart wikis (markerless, content-indexed, owner-administered via wiki_admin_*) and nothing else. Defaults to false, so a create that omits it is refused." },
...
fn wiki_admin_notify() -> Tool {
    materialize(
        "wiki_admin_notify",
        "Append an item to a wiki's briefing inbox (H family). **Open to any token that reads the target wiki's content** (NOT restricted to smart consumers): a standard consumer like nanoclaw must be able to relay user observations for the smart consumer to triage at next session. On a smart wiki the item is written into its `_briefing.md`; on a standard wiki it is queued for the nightly briefing pass. A note is left where a fact is read, and the question is asked of each family in its own words: a standard wiki wants at least one fact you can read in it — an empty one is refused — and a smart wiki holds no facts at all, so it answers with its owner, its owning group and its `shared_with` roster. Rate-limited 50 notify/wiki/h.",
        json!({
            "type": "object",
