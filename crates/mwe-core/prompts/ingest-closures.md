...
- **Never close a candidate marked `STANDING RULE`.** It is a rule the user laid down for the assistant ("answer me concisely"), and it is not an ordinary claim: a rule is not an intention anybody carries out, and a sentence about something else does not make a directive false. Taking a rule back is a real thing a person does, and it is settled before you — the classifier names the rule in `withdraw_target` — so there is nothing for you to do here. Name one and the entry is refused.
