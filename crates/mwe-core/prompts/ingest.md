...
**OPENING A NEW PAGE IS THE MOMENT TO BE CONSERVATIVE.** When `target_page` names a page that does NOT already exist, you are not filing a fact — you are asserting that a subject exists and describing what it covers, and the page you invent will be read long after this turn is forgotten. Two rules, both instances of DO NOT ADD SPECIFICITY (Part 2):

- The page's SUBJECT must be something the turn actually named. Never open a page for a person, a project, a client or an activity the message did not mention — and never promote what it did mention into something larger (an event is not a project; a purchase is not a collection; one complaint is not an area of work).
- The `page_description` must be answerable FROM THIS TURN. Describe what the page holds using the turn's own subject, at topic level. Do NOT name a kind of activity nobody described.
  - WRONG: the message complains that an assistant signed the sender up for the east fair at Granduardi → `target_page: "granduardi.md"`, `page_description: "Projects and activities relating to Granduardi"`. The name is anchored; the description is not. Nobody said there were projects, or activities, or that there is a body of work here at all — and the page then gets WRITTEN from that description.
  - RIGHT: `page_description: "the east fair at Granduardi, and what the sender has said about it"` — or, better, file the fact on an EXISTING page about the sender's work and let the nightly pass decide whether a page is warranted once there is more than one fact.

When in doubt between a new page and an existing one, prefer the existing one: an under-filed fact is moved by the nightly pass, an invented page and its description outlive the turn that caused them.
