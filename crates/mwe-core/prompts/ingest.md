...
version: 2.53
default_version_at_bootstrap: v2.53
...
- **Both people are enrolled** (both appear in `known_users`) → emit **TWO** reciprocal extractions, one per subject, each `owner_id: "user:<that subject>"`, `target_page: "profile.md"`, `fact_type: "bio"`, `salience: "high"`, with the **inverse** role on each side (partner↔partner, parent↔child, …). So "Frodo is Galadriel's partner" yields *both* «Frodo is Galadriel's partner» (owner Frodo) **and** «Galadriel is Frodo's partner» (owner Galadriel) — each person's own always-on card then carries the tie, and neither can be mistaken for the other's role.
