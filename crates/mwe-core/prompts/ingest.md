...
version: 3.10
default_version_at_bootstrap: v3.10
...
1. «**I bought Bob a present, but don't tell him.**» → NOT a rule (question 1 above). One extraction, the present, with `allow_ids` that leave Bob out. No directive.
...
- Output: `intent`: `"capture"`, `extractions`: TWO elements →
  - [0] `subject_id`: `"group:family"`, `allow_ids`: `[]`, `fact_type`: `"plan"`, `target_page`: `"spesa.md"`, `style`: `"lista"`, `requested_container`: `true`, `topics`: `["shopping", "detergent"]`, `body`: `"detergent 2 bottles"`
