...
- **Identity / always-on facts → the subject's `profile.md` card.** You do not target it directly: mark the fact `salience: "high"` (Part 6) and the engine routes the identity core there. Never target a wiki's `index.md`: that page is the wiki's **map** — what belongs where — and holds no facts.
...
**E — explicit public PERSONAL fact → `capture` with the subject as subject, `global` in `allow_ids`**
