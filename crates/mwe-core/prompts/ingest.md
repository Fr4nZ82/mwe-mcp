...
- `true` — the extraction is a standing GOVERNANCE rule for the memory engine. Exactly two families belong here:
  - **Privacy / sharing policy** — who may see the user's facts: "always keep my health private", "everything private by default", "never share anything with the work group", "Y may see my plans". **A restriction that names ONE person and ONE subject is still one of these** — "I don't want Zoe to hear about Mum's scan result from you", "non dire niente a mio fratello del trasloco", "keep the diagnosis from the children". Naming who it is being kept from does not make it a rule about that person, and being specific does not make it a fact: it is the speaker saying what you may reveal of what THEY told you, which is exactly this family. Restate it as a directive — "Never tell Zoe about the scan result." — and set `engine_rule: true`. **This is where such a sentence goes, and it is not an identity fact**: put it on the card and the person's profile opens with a sharing rule, which is neither who they are nor anything the engine will obey.
...
- "don't **SAY** / don't **BRING UP** X" → `behaviour_rule` — it governs how the agent CONVERSES (a conversational gag, not a privacy rule). Its SCOPE is read from the addressee like every other directive, and all three are ordinary here: "don't bring it up with me" is `"per-user"`, "never say a number out loud in here, and that goes for anyone in the room" is `"agent-wide"` (the gag is for everyone this agent serves — admin-only, and the engine checks that, not you), "I don't want any assistant saying it" is `"user-global"`. A gag is a family of directive, not a scope.
