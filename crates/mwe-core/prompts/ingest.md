...
WHEN BOTH HALVES OF THAT TEST ANSWER YES — «I prefer / I'd like» about HOW YOU TREAT ME. The test above offers its two questions as though only one could ever answer, and a directive dressed as a taste makes BOTH answer: "I prefer to get the summary as an audio message" is true about the speaker AND tells you how to reply. **An «I prefer» or an «I'd like» about how the assistant is to behave WITH THE SPEAKER is a `behaviour_rule`, and never a preference on their card.** The test that makes that decidable is WHO HAS TO ACT for the sentence to come true. Does it take the ASSISTANT behaving differently? Then it is a directive — whatever mood the verb is in, and whoever the grammatical subject is. Is it true whether or not any assistant exists? Then it is an ordinary fact. (This settles directive-vs-FACT only: a directive that turns out to govern SHARING or STORING is an `engine_rule` by the net routing rule below.) Grammar decides nothing here — "prefers to avoid the oven in hot weather" and "prefers to get the final verdict as an audio message" are the same sentence shape and land on opposite sides, because only the second one is a job for you. Restate it as the IMPERATIVE the deixis rule below asks for (`body`: "Send this user summaries and final verdicts as an audio message."), and read the scope off the addressee like every other directive: «I'd prefer the verdicts in audio» addresses nobody and is `"user-global"`, «I'd prefer YOU sent me the verdicts in audio» addresses the assistant in front of them and is `"per-user"`.

- THE FENCE, and it is the whole fence: "I prefer tea", "prefers to avoid the oven in hot weather", "listens to audiobooks while cooking", "Franz has the Max subscription". Every one of them is TRUE WITH NO ASSISTANT IN THE ROOM, so every one of them stays an ordinary fact and NONE of them is a `behaviour_rule`. A taste is not promoted into a directive by naming something you happen to be able to do.
