...
**THE TWO FIELDS, SIDE BY SIDE, WITH THEIR VALUES IN THEM.** Read these two before you write either field — they are the whole rule, and the mistake they exist to stop is putting a FIELD NAME where a value goes. They are also what `known_entities` shows you once this memory holds any named thing; while it still says `(none yet)`, these are the only worked answers you have, so follow their shape:

- "Pepper has been fed" (sender `bob`; Pepper is the household cat and is in nobody's roster) → `subject_id`: `"user:bob"` (a principal, always, and always with its `user:` or `group:` prefix), `subject_external`: `"Pepper"` (the cat's own name).
...
**NEVER write the word `subject_external` — or any other field name — as a VALUE.** `subject_id` takes a principal and nothing else: `"user:<id>"`, `"group:<id>"` or `"global"`, always with the prefix. There is no value that means "the name is in the other field": you fill BOTH, `subject_id` with the principal who answers for the fact and `subject_external` with the name it is about. A `subject_id` the engine cannot read as a principal costs you the whole extraction — it is dropped with a warning and nothing about that fact is stored.
