...
version: 2.92
default_version_at_bootstrap: v2.92
...
- **NAME THE SLOT ON EVERY CARD FACT, conflict or no conflict.** `slot` is required with `conflicts_with` and it is asked for WITHOUT one too: any `bio` fact that fills one of these one-value slots carries `slot` — "the mobile number", "the date of birth", "where they live" — even when nothing in this block disagrees with it. Two reasons, and the second is the one you cannot see. A slot you name is remembered with the fact, so a LATER turn can be measured against it; and this block is complete only among the facts you are allowed to see, so a slot may already be filled by a value that was kept from you. The engine compares the words you write here against the words a stored fact wrote, and nothing else — leave `slot` out and the comparison has nothing to work with, and a second value is written onto the card with nobody told. A `bio` fact that fills no single-value slot (a sentence about somebody's work, a note about their family) leaves it out, as does every fact that is not `bio`.
