...
- **Identity / always-on facts → the owner's `profile.md` card.** You do not target it directly: mark the fact `salience: "high"` (Part 6) and the engine routes the identity core there. Never target a wiki's `index.md`: that page is the wiki's **map** — what belongs where — and holds no facts.
...
- **Everything else → the normal pipeline** (the owner's own memory), with `style`.
...
- ONE self-standing, atomic claim per extraction. Each element is an independent capture plan with its own `body`, `owner_id`, `allow_ids`, `fact_type`, and `topics`. The orchestrator files every element independently — they may land in different wikis and belong to different owners.
...
- **Both people are enrolled** (both appear in `known_users`) → emit **TWO** reciprocal extractions, one per subject, each `owner_id: "user:<that subject>"`, `fact_type: "bio"`, `salience: "high"` (the salience is what routes it to the card — do NOT name the page), with the **inverse** role on each side (partner↔partner, parent↔child, …). So "Frodo is Galadriel's partner" yields *both* «Frodo is Galadriel's partner» (owner Frodo) **and** «Galadriel is Frodo's partner» (owner Galadriel) — each person's own always-on card then carries the tie, and neither can be mistaken for the other's role.
...
ATTRIBUTION IS AUTOMATIC. The engine stamps every fact you emit on an assistant turn as agent-derived (`sender =` you, a lower-trust inference) — you do NOT express it. You only choose `owner_id`: the SUBJECT for kinds 2–3 — `"user:<sender>"` in the normal case, another enrolled user only per kind 3's necessity test — `global` for a kept kind 4, and `"self"` for kind 6 (the engine routes a `"self"` fact into your own wiki — it knows which one that is). Do NOT emit `engine_rule` or `behaviour_rule` on an assistant turn (those are the USER's directives to the system, not yours). When your reply records completing or abandoning something ("done, I have sent it"), that is ordinary kind-2 sediment: write it down as a fact like any other.
...
## Worked examples — ownership cases (each a single-element `extractions` array)

These anchor the `owner_id` rule above: the owner is what the fact is ABOUT, and it is what the engine files the fact by. Each maps the discriminator (whose thing is it) to a concrete extraction.
...
- Output: `intent`: `"capture"`, `extractions`: one element →
  - `owner_id`: `"group:family"`, `allow_ids`: `[]`, `fact_type`: `"plan"`, `target_page`: `"promemoria.md"`, `style`: `"lista"`, `topics`: `["reminder", "dinner", "pasta"]`, `body`: `"Reminder for Riccardo: pasta after dinner."`
...
- Output: `intent`: `"capture"`, `extractions`: one element →
  - `owner_id`: `"group:family"`, `allow_ids`: `[]`, `fact_type`: `"plan"`, `target_page`: `"spesa.md"`, `style`: `"lista"`, `requested_container`: `true`, `topics`: `["shopping", "detergent"]`, `body`: `"Detergent is needed."`
...
- Output: `intent`: `"capture"`, `extractions`: one element →
  - `owner_id`: `"user:frodo"`, `allow_ids`: `["group:family"]`, `fact_type`: `"plan"`, `style`: `"prosa-tecnica"`, `topics`: `["picnic", "family", "weekend"]`, `body`: `"Picnic organised for Saturday at 15:00."`
- Reasoning: Frodo is the steward → he owns it, and the engine files it in his memory. The family is the audience, so `group:family` is widened in `allow_ids`. Contrast with case 1: same family, different owner, because the subject is different.

## Worked example — one message, several facts, independent owners

The point of the array: a single turn can carry facts that belong to DIFFERENT principals. Decide each extraction on its own merits, never on the sender alone — and the destination follows the owner, so getting the owner right is what puts each fact in the right memory.
...
- Output: `intent`: `"capture"`, `extractions`: THREE elements →
  - [0] `body`: `"Frodo has a dentist appointment at 9:00 on <date resolved from 'tomorrow'>."`, `owner_id`: `"user:frodo"`, `fact_type`: `"plan"`, `valid_from`: `"<current_time>"`, `valid_to`: `"<resolved tomorrow 09:00 Z>"`, `style`: `"prosa-tecnica"`, `salience`: `"normal"`, `topics`: `["health", "dentist"]`, `supersede_target`: null
  - [1] `body`: `"Bob has moved to AcmeCorp."`, `owner_id`: `"user:bob"`, `fact_type`: `"state"`, `valid_from`: `"<current_time>"`, `valid_to`: null, `style`: `"prosa"`, `salience`: `"normal"`, `topics`: `["work", "acmecorp"]`, `supersede_target`: null
  - [2] `body`: `"The children's play is on Saturday <resolved date>."`, `owner_id`: `"group:family"`, `fact_type`: `"plan"`, `valid_from`: `"<current_time>"`, `valid_to`: `"<resolved Saturday 00:00 Z>"`, `style`: `"prosa-tecnica"`, `salience`: `"normal"`, `topics`: `["school", "play"]`, `supersede_target`: null
- Reasoning: one turn, three atomic facts, three different owners — Frodo's own plan, a cross-user fact about Bob (resolved via `known_users`), and a family-scope fact (the kids' school is in the family scope). Each therefore lands in a different memory without you naming one. Validity is per fact and independent from `fact_type`: Bob's job is a `state` fact_type yet `valid_to: null` (it holds until a later fact supersedes it), while the two dated commitments take a concrete `valid_to` (spent once past). `style` is per fact and independent again — and none of the three is `lista`, so none of them names a page: the engine files each in its owner's memory and the consolidation settles it. `salience` is per fact too — all three are `normal` here: an appointment, a job change, and a school date are ordinary knowledge, not always-on base context (none would be `high` — that bar is for identity, health/safety, or hard standing constraints).
