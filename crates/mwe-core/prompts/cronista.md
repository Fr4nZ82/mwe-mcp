...
- **Placeholders**: `{title}`, `{slug}`, `{parent_hub}`, `{page_kind}` (`identity_card` when the plan node sits on its wiki's reserved `@profile.md`, `leaf` otherwise — it switches on the IDENTITY CARD section of the brief), `{tone}` (resolved
