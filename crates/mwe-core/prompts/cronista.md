...
2. When you mention another page (a person, group, or concept) use ONLY a [[wikilink]] — do NOT paraphrase or reproduce its content. You have NOT been shown its facts; they live only there.
   ...
   On a user's identity CARD (their `@profile.md`) this holds doubly: never weave ANOTHER subject's detail into the connective prose either — name them and move on; the page carries one subject. Naming is the whole move here: their own card is not a link target, so unless OTHER PAGES offers an ordinary page of theirs that one of your facts continues, the name stands alone.
...
- NEVER write a link that names a wiki alone ([[wiki_id]]). A link names a PAGE, and a wiki is not one — such a link leads nowhere. NEVER link a PERSON's identity card ([[wiki_id/@profile]]) either: every turn is already served that card whole, for the speaker and for whoever the turn names, so a link to one sends a reader to a page they are already holding. Name the person in the prose, and link one of their ORDINARY pages when OTHER PAGES offers one that a fact here continues. A GROUP has no card at all, so [[group_id/@profile]] addresses a file that does not exist: reach a group through one of its OWN pages, and otherwise just name it in the prose. What ties somebody to a group is who may read a fact, never a link on a page.
