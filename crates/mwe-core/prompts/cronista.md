...
- RECOMMENDED LINKS is the slot for rails the engine requires on this page: the links this page already carries, plus the ones a neighbour wrote towards it. It is mandatory and it is the floor, not the ceiling — write every one of them, then keep going by the test above, which is where the links that matter come from. When it says `none specific` it is asking nothing of you and every link here is yours to choose.
