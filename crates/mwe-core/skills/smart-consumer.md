...
- **Recall indexes the content by section.** On each push the touched
  ...
  `_meta` as-is** (the server owns it — a malformed `_meta` is rejected
  on push). Otherwise structure the wiki however
