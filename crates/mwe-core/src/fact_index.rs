...
pub struct FactIndexRow {
    ...
    /// that wrote the sentence, and `None` wherever [`Self::slot`] is `None`.
    ///
    /// The engine's slot comparison runs on this and not on the prose, because
    /// the prose is not the value: «Zoe can be reached on 07700 900314» and
    /// «Zoe's mobile number is 07700 900314.» are one number said twice, and
    /// word for word they are two answers. A row that carries no value falls
    /// back to comparing the bodies.
...
const ANOTHER_PERSONS: &[&str] = &[
    // English, in the possessive that gives it away. Both spellings of the
    // in-laws, because a memory is written by whoever is typing.
    "mother's",
    "mother-in-law's",
    "mum's",
    "mom's",
    ...
    "dog's",
    "cat's",
    // Italian, where the possession is spelled out in front. Both genders and
    // the plural where the language has one, because «di mia sorella» and «di
    // mio fratello» are two words for the same defect.
    "di mia madre",
    "di mio padre",
    "della mamma",
    "del papà",
...
pub struct NewFact {
    ...
    pub slot: Option<String>,
    /// See [`FactIndexRow::slot_value`]. Travels with [`Self::slot`].
    pub slot_value: Option<String>,
}
