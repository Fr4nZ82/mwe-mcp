...
See [../design-notes/logging.md](../design-notes/logging.md) for the
wiring.
...
| Navigator | `navigator` | The per-turn recall navigator: reads the root index + destination cards and decides which wikis/pages to open next ([recall pipeline](../design-notes/recall-pipeline.md)). Wants a **strong-but-cheap** model — per-turn latency/cost bound, but its link choices are the recall quality bar. | configured — the navigator funnel that consumes it is being built |
...
self-reorganization cycle ([../design-notes/rem-cycle.md](../design-notes/rem-cycle.md))
...
| `revisor_cap` | 30 | Max **act-first** `dedup_merge` merges the semantic revisor applies per cycle (born-applied receipt + 7-day revert window — see [rem-cycle.md](../design-notes/rem-cycle.md#revisor--conciliatore-sub-job)). |
...
slot — see [the recall pipeline](../design-notes/recall-pipeline.md) and
[the ingest pipeline](../design-notes/ingest-pipeline.md#the-recall-block--recalled-memory-the-rules-field-is-separate)).
...
the [document-ingest pipeline](../design-notes/document-ingest.md)
...
[`reindex::check_embedder_identity`](../design-notes/reindex-pipeline.md#embedder-identity-guard-roadmap-18g)
...
dashboard. See
[JWT & session model §recovery and 2FA](../design-notes/jwt-and-session-model.md#password-recovery-and-two-factor-authentication).
...
| `openai` | ❌ **gated** | — | Parses without error so an existing config does not break on upgrade, but `build_backend` returns `ConfigError::UnsupportedLlmBackend` — the operator's mistake surfaces **at startup**, not on the first request. The OpenAI adapter is not implemented today (planned — see the [roadmap](../roadmap.md)). |
...
- `navigator` (all profiles) — strong-but-cheap tier: it runs on every
  ...
  [extension](../planning/9_extensions.md)).
...
- [../design-notes/logging.md](../design-notes/logging.md) — how
  `logging.level` / `file_rotation` wire into the `tracing` subscriber.
- [../design-notes/rem-cycle.md](../design-notes/rem-cycle.md) — what a
