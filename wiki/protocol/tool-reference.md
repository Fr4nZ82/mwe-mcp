...
and [`mcp-dispatcher.md`](../design-notes/mcp-dispatcher.md)).
...
never optional. See
[`ingest-pipeline.md`](../design-notes/ingest-pipeline.md) for the
internal pipeline.
...
| `metadata` | object | no | Free-form. The dispatcher honours four keys: `disambig_choice` (string, the second-turn commit), `locale` (BCP-47 tag — an explicit LANGUAGE directive that overrides the per-user `enrollment_users.locale` default), `occurred_at` (ISO-8601 instant the message was originally uttered — the turn's semantic clock for backlog replays/imports: relative dates, validity windows and the due-soon horizon resolve against it instead of the server clock, while operational timestamps stay wall-clock; malformed → `400 invalid_input`), and `authored_refs` (array of plain `[[wiki_id/page]]` wikilinks — a smart consumer echoes the `authored_refs` its preceding `wiki_admin_push` returned so personal memory links to the project page instead of duplicating its body; blank/non-string entries ignored — group 17). |
| `attachments` | `array<{ catalog_id, kind?, caption?, description? }>` | no | Media riding this turn ([media pipeline](../design-notes/media-pipeline.md)). Bytes travel out of band: upload via `POST /media` first, pass the minted `catalog_id` here. Every id must parse (`400 invalid_input`), exist (`400 invalid_input`) and be readable by the effective sender (`403 sender_unauthorized`); the catalog row's `kind` is authoritative. Undescribed photos ride the classifier call as images; a `description` is trusted as the consumer's own vision. The captured fact's body carries the code-rendered `{{embed=…}}` marker; unclaimed attachments are filed by a deterministic fallback so catalogued media on an accepted turn is never dead — but `text` must still be non-empty (send the caption or a `[media]` placeholder for a captionless photo). |
...
- `rules` is **behaviour-only and structurally separate from
  `context_snippet`** ([ingest pipeline](../design-notes/ingest-pipeline.md)):
...
see the [failure ledger](../design-notes/rem-cycle.md#per-page-compile-failure-surfacing)).
...
See [`proposal-apply-engine.md`](../design-notes/proposal-apply-engine.md)
...
for the sender. See [`redaction-policy.md`](../design-notes/redaction-policy.md).
...
+ ACL post-filter. See
[`recall-pipeline.md`](../design-notes/recall-pipeline.md).
...
[`recall-pipeline.md`](../design-notes/recall-pipeline.md#consumer-facing-deep-recall--the-wiki_navigate-tool).
...
[media pipeline](../design-notes/media-pipeline.md)) actually run in
...
[document ingest](../design-notes/document-ingest.md).
...
- Corpus→pages import (a document becoming *its own pages* in its own
  ...
  ([extensions](../planning/9_extensions.md)); real bulk wiki import
...
[`jwt-and-session-model.md`](../design-notes/jwt-and-session-model.md)
and [`dashboard.md`](../design-notes/dashboard.md).
...
See [`smart-wikis.md`](../design-notes/smart-wikis.md).
...
- **caller may delete directly** — `acl::can_delete(fact.sender_id, caller,
  ...
  bytes ([capture pipeline](../design-notes/capture-and-dedup.md)) →
...
sweep, [redaction-policy](../design-notes/redaction-policy.md)).
...
apply). See [`agentic-chat.md`](../design-notes/agentic-chat.md). The
...
- [`mcp-dispatcher.md`](../design-notes/mcp-dispatcher.md) — dispatch,
  audit, and error-mapping internals.
- [`jwt-and-session-model.md`](../design-notes/jwt-and-session-model.md)
  — token claims, consumer class, sliding-TTL sessions.
- [`smart-wikis.md`](../design-notes/smart-wikis.md) — the
  smart-wiki model behind families H / J / K.
- [`ingest-pipeline.md`](../design-notes/ingest-pipeline.md) — what
  `wiki_ingest_message` runs internally.
- [`agentic-chat.md`](../design-notes/agentic-chat.md) — the dashboard
  agentic loop.
