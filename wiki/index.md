...
> code updates the relevant page in the same commit. For *forward-looking*
> work — what we still plan to build — see [`roadmap.md`](roadmap.md) and
> its detail pages under [`planning/`](planning/).
...
> See [`../CLAUDE.md`](../CLAUDE.md) for the full glossary.
...
frontmatter marks the maturity of that area. For what is *planned* next,
...
### `design-notes/`
- [wiki-filesystem-ssot.md](design-notes/wiki-filesystem-ssot.md) — on-disk
  layout of `<workdir>/wikis/`, the `WikiMeta` frontmatter schema, the
  atomic write protocol (`mwe-core::wiki`).
...
### forward work
- [roadmap.md](roadmap.md) — the single list of what is still to build
  (only remaining work), one detail page per area under
  [`planning/`](planning/).
- [logs.md](logs.md) — the append-only decision log.

---

This wiki is the SSOT: it aims to cover the full current state of the
code — the data model, the runtime behaviour, the protocol surface, the
configuration, and worked examples. Forward-looking work (what we plan to
build next) lives in [`roadmap.md`](roadmap.md) and the detail pages under
[`planning/`](planning/); the decision log is [`logs.md`](logs.md). The
