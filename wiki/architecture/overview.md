...
| `archive` | REM-emitted `archive_proposals` (schema in [`engine-db-and-migrations.md`](../design-notes/engine-db-and-migrations.md)) — `emit_archive_proposal` writes one row per stale filesystem path with a `reason` string (`no_recall_hit_365d`, `no_modify_180d`); coarse idempotency probe `has_active_archive_proposal` skips dupes. Approval flow + apply step (filesystem move + `wiki_forget` cascade) are not yet wired into the dashboard archive view. Backing module for the REM `run_archive_detector` sub-job. | implemented |
| `audit` | Writer + searcher for `tool_executions` (schema in [`engine-db-and-migrations.md`](../design-notes/engine-db-and-migrations.md)). `ToolExecutionInput` builder, SHA-256-hashed `args_hash`, `SearchFilters` (sender/tool/date range/result_status/top_k), `ResultStatus::{Success, Error}`. Backs the `tool_log_search` MCP tool + the dispatcher's per-call audit row. | implemented |
...
| `config` | Loader for `<workdir>/mwe-mcp.config.yaml`. Materialises `logging.level` (`info` / `debug`) and the `llm:` section — five canonical functions (`hub_writer` / `ingest` / `rem_promotions` / `rem_dedup_semantic` / `cronista`) with `LlmFunctionConfig::build_backend` (Ollama, Anthropic, Gemini; `openai` gated by `UnsupportedLlmBackend`) and env-var override `MWE_LLM_<FUNC>_<MODEL|BACKEND|API_KEY_ENV|BASE_URL>`. Cloud slots (`anthropic`, `gemini`) share the `MissingApiKeyEnv` path via the `resolve_cloud_api_key` helper — `api_key_env` absent/unset/empty errors at config-build time, never at first request; env-injectable `build_backend_with_env` for tests. Unknown top-level keys round-trip via `Config::extra` so the rest of the schema (embedding, rem, budget, …) can land without breaking parse. Malformed files are fatal at startup. | partial |
...
| `promote` | Per-kind apply/revert handler for the `wiki_promote` structure proposal kind + `emit_paragraph_to_file` emitter. Two variants share the chassis arm: **`paragraph_to_file`** (default — move N facts from one page of a wiki to another page of the same wiki) and **`file_to_subwiki`** (turn an entire page into a new sub-wiki whose `index.md` carries the content verbatim; new id derived via `WikiId::child_of`). Both preserve every `fact_id` verbatim — what changes is `fact_index.source_path` (always) and `wiki_id` (only for `file_to_subwiki`). | implemented |
...
3. **MCP dispatcher** — [`mcp` module](../../crates/mwe-mcp-server/src/mcp/),
   ...
   dedicated [dispatcher design note](../design-notes/mcp-dispatcher.md)
...
4. **HTTP media byte pair** — [`http_media` module](../../crates/mwe-mcp-server/src/http_media.rs),
   ...
   SSOT: [`media-pipeline.md`](../design-notes/media-pipeline.md).
...
| `/tokens`, `/tokens/issue`, `/tokens/revoke`, `/tokens/delegation/:consumer_id` | Issue (with optional consumer + delegation list — the `X-MWE-Act-As` model is in [`jwt-and-session-model.md`](../design-notes/jwt-and-session-model.md)), list blacklist, revoke, edit delegation |
...
See [`design-notes/dashboard.md`](../design-notes/dashboard.md) for
the auth model + identity console and
[`design-notes/dashboard-memory-mvp.md`](../design-notes/dashboard-memory-mvp.md)
...
```
<workdir>/
├── engine.db                 # sqlite, see design-notes/engine-db-and-migrations.md
...
SSD, or a tmpfs for tests. **It is never** `mwe-mcp/wiki/`, which is
this engineering wiki — see [`../../CLAUDE.md`](../../CLAUDE.md)
terminology section for the full disambiguation.
...
- [`lockfile`](../../crates/mwe-core/src/lockfile.rs) holds a kernel
  ...
  process (see [`design-notes/single-writer-lockfile.md`](../design-notes/single-writer-lockfile.md)
...
- [`wal`](../../crates/mwe-core/src/wal.rs) journals every step of a
  ...
  startup (see [`design-notes/applicative-wal.md`](../design-notes/applicative-wal.md)).
...
- **Memory engine + dashboard.** The MCP tool surface is wired through
  the [`mcp` dispatcher](../design-notes/mcp-dispatcher.md) over HTTP
  ...
  ([`reindex`](../design-notes/reindex-pipeline.md)), the dashboard
  memory browser
  ([`dashboard-memory-mvp.md`](../design-notes/dashboard-memory-mvp.md)),
...
- **REM + structure proposals + smart wikis.** The proposal chassis
  ...
  [`rem-cycle.md`](../design-notes/rem-cycle.md)) wires the full sub-job
