...
[`atomic_write`](wiki-filesystem-ssot.md) protocol as every page
...
re-running just rebuilds the plan from scratch. That keeps the project-wide
[filesystem-is-SSOT invariant](wiki-filesystem-ssot.md) intact. The
`_plan/` directory needs **no new exclusion machinery**: it carries no
`_meta.md`, so [`tree.walk`](wiki-filesystem-ssot.md) skips it as a
...
[roadmap](../roadmap.md) (prose is the accuracy mechanism). On a user's
...
> organic forgetting ([roadmap group 11](../planning/11_forgetting.md)).
