...
- A local Ollama (or compatible) endpoint if you want to exercise the
  ...
  [`llm-functions.md`](../design-notes/llm-functions.md) for the per-slot
  roster, and [`logging.md`](../design-notes/logging.md) for the wiring.
...
[`design-notes/dashboard-frontend.md`](../design-notes/dashboard-frontend.md)
...
- **`backup`** — hot workdir snapshot for backup / disaster recovery:
  ...
  [backup & DR](../design-notes/backup-and-dr.md).
- **`rem run-cycle`** — drive one full REM cycle synchronously and
  ...
  [`rem-cycle.md`](../design-notes/rem-cycle.md) (the code roster is
...
- **`recall eval`** — replay a YAML gold set against the workdir and
  ...
  definitions: [`recall-pipeline.md`](../design-notes/recall-pipeline.md#the-recall-eval-harness--recall_eval).
...
[`design-notes/logging.md`](../design-notes/logging.md) for the full
...
[`dashboard-frontend.md`](../design-notes/dashboard-frontend.md) for
the editor surface.
...
- **Boot-time LLM slot health check fails.** `mwe-mcp serve` calls
  ...
  [`rem-cycle.md` §LLM-error semantics](../design-notes/rem-cycle.md#llm-error-semantics).
