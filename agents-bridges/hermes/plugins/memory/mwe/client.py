"""Minimal MCP client for the mwe provider (Streamable HTTP, stdlib-only).

Vendored from `agents-bridges/_harness/mwe_client.py` (the reference
client) so the plugin directory is self-contained when symlinked into
`$HERMES_HOME/plugins/`. Keep the two in sync when the transport
changes.
"""
...
class MweClient:
    ...
    The provider keeps one instance per delegated sender (the per-sender
    pool): the act-as identity is fixed at construction, mirroring MCP
    clients that can only set headers at connect time.
    """
