...
| `rem.policy.auto_promote_min_page_facts` | int \| _unset_ | _unset_ → `RemPolicy` default `8` | Min active facts on a **`prosa`** page (page mass) before the split pass shows it to the LLM. The only deterministic gate — a resource pre-filter, not a semantic one. Lower it to exercise auto-promotion in a small deployment. |
| `rem.policy.auto_promote_min_page_facts_technical` | int \| _unset_ | _unset_ → `RemPolicy` default `16` | The same floor for a **`prosa-tecnica`** page, which is scanned by points rather than read as a thread and so tolerates more mass. A **`lista`** page has no floor at all and is never split by mass: it is consulted, and half a list answers nothing. |
...
| `auto_promote_min_page_facts` | 8 | Min page mass before the split pass shows a **`prosa`** page to the LLM (the only deterministic gate — no recall floor). **YAML: `rem.policy.auto_promote_min_page_facts`.** |
| `auto_promote_min_page_facts_technical` | 16 | The same floor for **`prosa-tecnica`**; **`lista`** is never split by mass. **YAML: `rem.policy.auto_promote_min_page_facts_technical`.** |
...
| Key | Type | Default | Notes |
|---|---|---|---|
| `recall.recall_top_k` | int \| _unset_ | _unset_ → `10` | Flat-slot size: vector-recall hits in the classifier's context block, and the number of entry doors the navigator is given. |
| `recall.nav_seed_depth` | int \| _unset_ | _unset_ → `30` | How far past the flat slot the entry fan may read the same ranking to find a door on a page it has not already reached. Two hits on one page are one door; this fills the freed slot. Clamped up to `recall_top_k`. |
...
| `recall.max_sender_identity_chars` | int \| _unset_ | _unset_ → `2500` | Failsafe on the `WHO IS SPEAKING` identity card (whole-paragraph fitting). Curate the page, not this number; `0` serves the label line alone. |
