...
| Snapshot writes (`_snapshots/proposal/…`) | — | Reserved for the `bundle` multi-op kind handler, which is the only kind where N filesystem writes must roll back together. Not implemented today (planned — see the roadmap). |
