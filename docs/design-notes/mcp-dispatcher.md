...
| `wiki_lint` | 🟡 partial — the check roster and which of them actually run are both read off [`mwe_core::lint`](../../crates/mwe-core/src/lint.rs) (`Check::all()` / `is_implemented()`); [`mcp-tools.md`](../protocol/mcp-tools.md) narrates it and can lag | [`mwe_core::lint`](../../crates/mwe-core/src/lint.rs) |
...
| `recall_core_global` | ✅ full — smart only | [`mwe_core::smart::recall_core_global`](../../crates/mwe-core/src/smart.rs); thin wrapper over `recall::wiki_search` filtered on the fact's **subject** (`subject_id = user:<caller>` — facts *about* the caller, wherever they are filed; it is a predicate on the subject, not on which wiki holds the fact); smart-wiki material is absent by construction (it lives in `wiki_sections`, which this view never queries), so there is no post-filter and no overfetch — the excluded smart `wiki_type`s are echoed in `filter_applied` as a policy statement; clamps `limit ∈ [1, 20]` |
