...
meta flag — smart section rows carry `sender_id = NULL` and subject = the
scope principal, so no sender/subject gate would catch them). Smart wikis
...
`POST /facts/:id/validity` — subject-or-admin, standard-wikis only,
