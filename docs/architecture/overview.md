...
| `capture_buffer` | The pre-compilation staging area for the standard-wiki path: `wiki_ingest_message` parks each classified claim here as a buffered capture instead of writing straight to the published page, and the light dream drains it into `fact_index`. A buffered row names **no wiki and no page** — a claim waits here precisely because nobody has placed it, and the drain places it. Backs the mid-range fresh-captures recall slot (`recall_fresh_captures`). | implemented |
...
| `compiler` | Narrative prose stage — Il Cronista (the `cronista` LLM slot) and the deterministic record renderer. Consumes the planner's `CompilationPlan` and, per dirty page, compiles the assigned facts into cohesive prose (`compile_leaf_page`) or renders a `lista` page with no LLM. No page lists other pages (2026-08-19). Runs in the dream compile pass; recall does not yet serve the compiled prose (see `narrative-buffer.md`). | implemented |
...
| `dream_light` | The light dream — the frequent, cheap half of the two-dream cadence. Drains the captures queue in two halves with the compilation plan between them: `screen_queue` folds the duplicates (against the facts about the same subject AND against the other claims in the queue) and projects the rest for the plan with no page; `materialise` then writes each placed claim into `fact_index` addressed to the page the plan chose, moments before the compile writes that page. `drain_deterministically` does both with no model at all. A claim the plan cannot place keeps waiting. | implemented |
...
| `lint` | Backing for `wiki_lint` MCP tool — 7 advertised `Check` variants with `is_implemented()` flag. `MarkerMalformed` (re-runs the parser per page), `OrphanFacts` (fact_index rows whose marker is missing on disk), `MetaInvalid` (unparseable `_meta.md`) and `EmbedMissing` are implemented; the other 3 (`BrokenCrosslinks`, `AclInconsistent`, `SupersededChain`) advertise a zero-count summary entry and are not yet implemented. Output sorted `(severity desc, check asc, wiki_id asc, fact_id asc)` for deterministic diffs. | partial |
...
| `media` | Media catalog + content-addressed blob store behind `{{embed=<catalog_id>}}` keys: `media_catalog` DAL (the `fact_index` twin — ACL triple, kind/MIME/size, caption/description), `store_media` (sha256 addressing under `<workdir>/media/<aa>/<sha>`, blob-before-row write order, per-(hash, subject) idempotent dedup), server-side `catalog_id` minting with the closed English kind vocabulary (`photo`/`video`/`audio`/`doc`, producer-enforced), `row_visible_to` (`can_read` semantics, no admin bypass), monotone `widen_acl` (the fact-link union), `inline_safe` (the MIME safety call shared by both serving mounts). Backs `POST /media` / `GET /media/<id>` + the dashboard alias + export bundling + the `embed_missing` lint. Design SSOT: `media-pipeline.md`. | implemented |
| `meta_annotate` | Deterministic, no-LLM compile sub-stage that derives card annotations from `fact_index` — each **page**'s testata `keywords["topics"]`, which is the only card a turn is ever shown, plus each wiki's `_meta.md` `keywords["topics"]` and one-line abstract, which are **write-side** vocabulary for the filer. Nothing here feeds the read side a wiki or a list of them: [there is none](../design-notes/recall-pipeline.md). | implemented |
| `parser` | Inline marker `{{subject=... allow=... f=... sender=...}}…{{/}}` parser. `owner=` is a permanent read-only alias of `subject=` — accepted on every read, never emitted, and `subject=` wins when a region carries both. | implemented |
| `planner` | Topology stage of the narrative compiler. Turns the flat `fact_index` into a `CompilationPlan`: a page graph where every fact lives on exactly one page (one-fact-one-page), and a persistent `ConceptRegistry` keeps a concept page from being re-invented run-to-run. Never writes prose itself. | implemented |
| `promote` | Per-kind apply/revert handler for the `wiki_promote` structure proposal kind + `emit_paragraph_to_file` emitter. The structural variants: **`paragraph_to_file`** (default — move N facts from one page of a wiki to another page of the same wiki), **`pages_to_subwiki`** (a group of pages that are one subject becomes a new sub-wiki, each page carried under its own name; new id derived via `WikiId::child_of`), and **`pages_move_wiki`** (pages file into a sub-wiki that already exists). **`file_to_subwiki`** — the single-page emergence, no emitter but applicable by an operator from the dashboard form — carries the promoted page over under its own name too; the new wiki gets its `_meta.md` and that page and nothing else, and a receipt written before 2026-08-03 (when the page's bytes *were* the new wiki's `index.md`) stays revertible in its old shape. All preserve every `fact_id` verbatim — what changes is `fact_index.source_path` (always) and `wiki_id` (whenever a page crosses a wiki line). | implemented |
...
| `smart` | Smart-consumer bootstrap helpers backing the smart-consumer MCP family. `bootstrap(pool, tree, caller, req) -> BootstrapResponse` surfaces every smart-family wiki the caller owns (filter via each `_meta.md`'s per-wiki smart flag — no registry probe — + `tree.resolve_scope_principal(meta) == User(caller.sender_id)`), aggregating `briefing::counts_by_kind` + `briefing::list_items` (pending only, capped per request) + latest `wiki_admin_op_log` row per wiki; sorts `(matches_project_hint desc, last_op_log_ts desc, wiki_id asc)`. `recall_core_global(pool, tree, embedder, caller, sender_groups, req) -> RecallCoreGlobalResponse` is a thin wrapper over `recall::wiki_search` filtered on the **fact's subject** (`subject_id = user:<caller>`) — facts *about* the caller wherever they are filed. The predicate is on `fact_index.subject_id`, not on which wiki holds the row: a wiki whose proprietor is the caller can perfectly well hold facts about other people, and those are not returned. `limit` is clamped `∈ [1, 20]` and honoured as-is (no overfetch, no post-filter); the smart-family `wiki_type`s are echoed in `filter_applied.excluded_wiki_types` as the policy statement that smart-wiki material is not in this view. Both gated on `consumer_class=smart` (`SmartError::RequiresSmart`). `read_only` annotation on the two MCP tools. | implemented |
...
5. **HTTP skill catalog** — [`http_skills` module](../../crates/mwe-mcp-server/src/http_skills.rs),
   ...
   `/mcp`. Every skill is bundled and public, so the HTTP path needs
   no JWT context and serves the same set as `skill_list`.
