...
| **Ingest inside `wiki_ingest_message`** *(default conversational turn)* | mwe-mcp | internal LLM (`ingest` function) | mwe-mcp owner (local or API) | Zero if local; a fraction of a cent per call on a small API model. Up to three calls on the `ingest` slot per turn — the classifier always, the closure confirmer only when the turn closes something, and the reconciler after the navigator has read. |
...
The full smart-consumer flow (bootstrap, scenarios, smart-wiki
